...
const researchedAdmissionSources: Record<string, ResearchedAdmissionSource> = {
  ...
  trungvuong: {
    ...
    note:
      ...
      ' Superseded (2026-09-04 batch): verified-calculator upgrade shipped for 16/16 majors, Phương thức 2 (THPT-exam) only, using the 2025 cycle instead of 2026. Official Thông báo 387/TB-ĐHTV (09/06/2025, signed + sealed by Hiệu trưởng) — mirrored on tuyensinh247\'s CDN because tv-uni.edu.vn itself is unreliable to fetch directly — gives the formula "ĐXT = TN1+TN2+TN3+Điểm ưu tiên (nếu có)" and the full mã-ngành/tổ hợp table for 16 majors. The 2025 per-major cutoff table (15.00-19.00/30) itself came from 3 independent secondary sources (tuyensinh247, fptshop.com.vn, trangedu.com) that matched exactly on all 16 majors, because tv-uni.edu.vn\'s own điểm-chuẩn-2025 page no longer carries the full per-major table (overwritten by later cycles) — same cross-check technique already accepted for HAT/HLUV/HBU. IMPORTANT gotcha caught and avoided: the tuyensinh247 CDN image at the same slug now shows 2026 data (17 majors, including a new Y khoa major absent from the 2025 official notice) — verified this by cross-referencing the 16 mã ngành in the 2025 signed PDF before trusting the aggregator numbers, so the year mixup did not make it into the calculator. Excluded 2 combinations with no matching SubjectId (D04 Tiếng Trung, DD2 Tiếng Hàn) and 2 combinations of unclear subject composition (B01, D02, Tourism major only) — affected majors still compute via their other published combinations. Priority points use the current national framework (school only says "theo quy chế tuyển sinh của Bộ Giáo dục và Đào tạo", no school-specific table — judgment call, same precedent as DNU/TUEBA/PVU/HUST). See normalized/runtime-source-snapshot/trungvuong/sources.ts. This entry is unused because trungvuong is now in explicitRuntimeSchoolIds; kept only as a research trail.',
  },
  tucst: {
  ...
  dsu: {
    ...
    checkedAt: '2026-08-24',
    note:
      'Superseded (2026-09-04 batch): verified-calculator upgrade shipped for Phương thức mã 100 (điểm thi TN THPT, ngành Quản lý TDTT 7810301 only). Batch expand-06 (2026-08-24) found only the top-level admission page (2026 notice titles, no extractable table). This batch found the 2025 threshold decision (Quyết định 1088/QĐ-TDTTĐN-HĐTS, 22/8/2025, PDF chữ ký + con dấu, đọc trực tiếp không cần vision — bảng "Mức điểm chuẩn trúng tuyển" đầy đủ 3 ngành x 4 phương thức) and the matching 2025 admission notice (Thông báo 247/TB-TDTTĐN, 07/3/2025 — tổ hợp B03/C14 cho phương thức 100, chỉ áp dụng ngành Quản lý TDTT) and quy chế tuyển sinh (Quyết định 577/QĐ-TDTTĐN, 12/5/2025 — Điều 7 tự công bố bảng điểm ưu tiên KV/ĐT + công thức giảm dần, không phải judgment call). Cross-referenced against the 2026 admission notice (Thông báo 58/TB-TDTTĐN, jsst.edu.vn mirror) to confirm mode 100/200 pairs only with non-năng-khiếu tổ hợp across both years. See normalized/runtime-source-snapshot/dsu/sources.ts. This entry is unused because dsu is now in explicitRuntimeSchoolIds; kept only as a research trail.',
  },
  eaut: {
  ...
  hbu: {
    ...
    checkedAt: '2026-08-24',
    note:
      "Superseded (batch expand-28, 2026-09-04): verified-calculator upgrade shipped for 18/21 majors (THPT-exam branch only). The 2026-08-24 blocker (linked PDF not extracted) was resolved by switching to the 2025 cycle instead: root domain daihochoabinh.edu.vn (including the original 2025 threshold announcement) is connection-refused/timed-out from this research environment (both WebFetch and chrome-devtools direct navigation), but the tuyensinh.daihochoabinh.edu.vn subdomain (same school, different server) IS reachable — used for the official combination/mã-ngành table (WordPress-embedded image, read via vision through chrome-devtools) and the formula page (\"ĐXT = TĐ1 + ĐUT\"). The 2025 threshold table itself came from 2 independent secondary sources (tuyensinh247 + navigates.vn, cross-checked further against giaoduc.net.vn/Sforum) that matched exactly across all 21 majors and included tuyensinh247's direct confirmation that published thresholds already include priority points — same cross-check technique already accepted for HAT/HLUV when the official domain was unreachable. Excluded 3/21 Thiết kế majors (năng khiếu \"Vẽ\" combos V00-V03, no SubjectId, and the official legend itself is missing V02/V03 definitions) and the D65 combination for Ngôn ngữ Trung Quốc (Tiếng Trung has no SubjectId; 3/4 of that major's combinations still work). Priority points use the current national framework (school does not publish its own table, judgment call — same precedent as DLA/VTTU/TUEBA/PVU/HTU/TUMP/NAEM). See normalized/runtime-source-snapshot/hbu/sources.ts. This entry is unused because hbu is now in explicitRuntimeSchoolIds; kept only as a research trail.",
  },
  hca: {
...
const explicitRuntimeSchoolIds = new Set(['vgu', 'hpu2', 'apd', 'eiu', 'fbu', 'fpfu', 'ntuhn', 'tbdu', 'thanhdo', 'tnue', 'tnufl', 'tnus', 'tuaf', 'uhd', 'umt', 'utm', 'utt', 'eaut', 'vwa', 'hau', 'tbu', 'ltvuni', 'dhv', 'ush', 'hcmupes', 'hnmu', 'cmcu', 'vttu', 'hbu', 'vhs', 'vya', 'trungvuong']);
