...
describe('Dai Nam THPT exam threshold eligibility 2026', () => {
  ...
  it('marks totals below 15/30 as ineligible', () => {
    ...
    expect(result.evidence).toContainEqual(expect.objectContaining({ sourceId: 'dainam-admission-portal-2026' }));
  });
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'dainam', { context: a00Context }).status).toBe('eligible');
    expect(evaluateSchools(profile, ['dainam'], { dainam: a00Context })[0].status).toBe('eligible');
  });
});
