...
describe('UFLS THPT threshold eligibility 2026', () => {
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'uflsudn', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['uflsudn'], { uflsudn: a00Context })[0].status).toBe('ineligible');
  });
});
