...
describe('HALONGU PT1 THPT threshold eligibility 2026 (nhóm ngành ngoài sư phạm)', () => {
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'halongu', { context: a00Context }).status).toBe('eligible');
    expect(evaluateSchools(profile, ['halongu'], { halongu: a00Context })[0].status).toBe('eligible');
  });
});
