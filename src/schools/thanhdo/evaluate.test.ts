...
describe('ThanhDo THPT-exam-route eligibility 2026', () => {
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'thanhdo', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['thanhdo'], { thanhdo: a00Context })[0].status).toBe('ineligible');
  });
});
