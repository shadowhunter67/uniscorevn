...
describe('NCTU THPT baseline eligibility 2026', () => {
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'nctu', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['nctu'], { nctu: a00Context })[0].status).toBe('ineligible');
  });
});
