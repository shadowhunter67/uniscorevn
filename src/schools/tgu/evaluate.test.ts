...
describe('TGU THPT baseline eligibility 2026', () => {
  it('marks profiles below the common 15/30 baseline as ineligible', () => {
    ...
    expect(result.evidence).toContainEqual(expect.objectContaining({ sourceId: 'tgu-admission-info-2026' }));
  });
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'tgu', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['tgu'], { tgu: a00Context })[0].status).toBe('ineligible');
  });
});
