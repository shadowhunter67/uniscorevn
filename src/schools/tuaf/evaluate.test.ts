...
describe('TUAF THPT baseline eligibility 2026', () => {
  it('marks profiles below the 16/30 floor as ineligible', () => {
    ...
    expect(result.evidence).toContainEqual(expect.objectContaining({ sourceId: 'tuaf-admission-info-2026' }));
  });
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'tuaf', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['tuaf'], { tuaf: a00Context })[0].status).toBe('ineligible');
  });
});
