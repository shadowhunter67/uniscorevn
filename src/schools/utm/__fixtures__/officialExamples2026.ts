...
import type { UtmThptExamEvaluationContext } from '../evaluate';

/**
 * Tier C — nguồn `utm-threshold-2026` công bố TRỰC TIẾP con số ngưỡng "Các ngành đào tạo: từ 15
 * điểm" (Tier B ingredient) nhưng im lặng về điểm ưu tiên khu vực/đối tượng => nhánh exact cộng
 * điểm ưu tiên theo judgment call chuẩn quốc gia (Điều 7 Thông tư 08/2022/TT-BGDĐT, `priority.ts`)
 * — expected tính TAY (không gọi calculator) nên xếp Tier C, có `derivation`.
 */
export const utmThptExamExactGoldenCases: GoldenAdmissionCase<
  { profile: ApplicantProfile; context: UtmThptExamEvaluationContext },
  { eligible: boolean; raw30: number; finalScore30: number }
>[] = [
  {
    id: 'utm-2026-exact-threshold-pass-no-priority',
    schoolId: 'utm',
    methodId: 'utm-thpt-exam-exact-2026',
    year: 2026,
    tier: 'C',
    sourceId: 'utm-threshold-2026',
    sourceNote: 'Ngưỡng công bố trực tiếp "các ngành đào tạo: từ 15 điểm"; không khai điểm ưu tiên -> điểm ưu tiên = 0.',
    derivation: `
  ...
  {
    id: 'utm-2026-exact-threshold-fail-below-15',
    schoolId: 'utm',
    methodId: 'utm-thpt-exam-exact-2026',
    year: 2026,
    tier: 'C',
    ...
    derivation: `
      ...
      Ngưỡng so với TỔNG THÔ (đúng văn bản, judgment call không thay đổi vế so sánh ngưỡng) -> 14,00 < 15,00 -> chưa đạt.
  ...
  {
    id: 'utm-2026-exact-priority-reduction-above-22-5',
    schoolId: 'utm',
    methodId: 'utm-thpt-exam-exact-2026',
    year: 2026,
    tier: 'C',
    ...
    sourceNote: 'Áp dụng công thức giảm điểm ưu tiên chuẩn quốc gia khi tổng thô >= 22,5 (Điều 7 Thông tư 08/2022/TT-BGDĐT).',
    derivation: `
