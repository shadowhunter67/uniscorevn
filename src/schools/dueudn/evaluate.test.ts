...
describe('DUE THPT threshold eligibility 2026', () => {
  it('requires a selected subject combination', () => {
    const profile: ApplicantProfile = { thpt: { scores: { math: 8, physics: 7, chemistry: 6.5 } } };

    const result = evaluateDueudnThptExamAdmission(profile);

    expect(result.eligibility?.status).toBe('unknown');
    expect(result.missingRequirements).toContainEqual(expect.objectContaining({ kind: 'school-context', code: 'dueudn-subject-combination' }));
  });

  it('reports missing THPT subject scores', () => {
    const profile: ApplicantProfile = { thpt: { scores: { math: 8, physics: 7 } } };

    const result = evaluateDueudnThptExamAdmission(profile, a00Context);
  ...
  it('marks totals below the lowest published threshold as ineligible', () => {
    ...
    expect(result.evidence).toContainEqual(expect.objectContaining({ sourceId: 'dueudn-quality-threshold-2026' }));
  });
  ...
  it('routes through generic evaluateSchool and evaluateSchools adapters', () => {
    ...
    expect(evaluateSchool(profile, 'dueudn', { context: a00Context }).status).toBe('ineligible');
    expect(evaluateSchools(profile, ['dueudn'], { dueudn: a00Context })[0].status).toBe('ineligible');
  });
});
